    ...
    with st.container():
        ...
        if st.session_state.data is not None:
            ...
            with st.container():
                ...
                st.session_state.std = round(cgm.sd(),2)
                st.session_state.ea1c = round(cgm.eA1c(), 2)
                st.session_state.trange = cgm.time_in_range()
                trange_str = f'{st.session_state.trange}%'
                st.session_state.thyper = cgm.hyper_time()
                thyper_str = f'{st.session_state.thyper}%'
                st.session_state.thypo = cgm.hypo_time()
                thypo_str = f'{st.session_state.thypo}%'
                st.session_state.iqr = cgm.inter_qr()
                st.session_state.intersd = round(cgm.interdaysd(), 2)
                st.session_state.intrasd = round(cgm.intradaysd()[0], 2)
                st.session_state.mage = round(cgm.MAGE(), 2)
                ...
                if cgm2 is not None:
                    ...
                    st.session_state.intersd_delta = round(st.session_state.intersd-cgm2.interdaysd(), 2)
                    st.session_state.intrasd_delta = round(st.session_state.intrasd-cgm2.intradaysd()[0], 2)
                    ...
                    col1.metric(label="Number of measurements", value=st.session_state.n_data, delta=st.session_state.n_data_delta)
                    col2.metric(label="Average Glucose", value=st.session_state.avg, delta=st.session_state.avg_delta, delta_color="inverse")
                    col3.metric(label="Standard Deviation (SD)", value=st.session_state.std, delta=st.session_state.std_delta, delta_color="inverse")
                    col4.metric(label="Estimated A1c", value=st.session_state.ea1c, delta=st.session_state.ea1c_delta, delta_color="inverse")
                    col1.metric(label="Time in range", value=trange_str, delta=st.session_state.trange_delta)
                    col2.metric(label='Time in hyper', value=thyper_str, delta=st.session_state.thyper_delta, delta_color="inverse")
                    col3.metric(label='Time in hypo', value=thypo_str, delta=st.session_state.thypo_delta, delta_color="inverse")
                    col4.metric(label='Interquartile range', value=st.session_state.iqr, delta=st.session_state.iqr_delta, delta_color="inverse")
                    col1.metric(label="Interday SD", value=st.session_state.intersd, delta=st.session_state.intersd_delta, delta_color="inverse")
                    col2.metric(label="Intraday SD", value=st.session_state.intrasd, delta=st.session_state.intrasd_delta, delta_color="inverse")
                    col3.metric(label="MAGE", value=st.session_state.mage, delta=st.session_state.mage_delta, delta_color="inverse")
                    col4.metric(label="J-Index", value=st.session_state.jindex, delta=st.session_state.jindex_delta, delta_color="inverse")
                    col1.metric(label="LBGI", value=st.session_state.lgbi, delta=st.session_state.lgbi_delta, delta_color="inverse")
                    col2.metric(label="HBGI", value=st.session_state.hbgi, delta=st.session_state.hbgi_delta, delta_color="inverse")
                    col3.metric(label="CONGA24", value=st.session_state.conga, delta=st.session_state.conga_delta, delta_color="inverse")
                    col4.metric(label="GMI", value=st.session_state.gmi, delta=st.session_state.gmi_delta, delta_color="inverse")
                
                else:

                    col1.metric(label="Number of measurements", value=st.session_state.n_data)
                    col2.metric(label="Average Glucose", value=st.session_state.avg)
                    col3.metric(label="Standard Deviation (SD)", value=st.session_state.std)
                    col4.metric(label="Estimated A1c", value=st.session_state.ea1c)
                    col2.metric(label='Time in hyper', value=thyper_str)
                    col3.metric(label='Time in hypo', value=thypo_str)
                    col4.metric(label='Interquartile range', value=st.session_state.iqr)
                    col1.metric(label="Interday SD", value=st.session_state.intersd)
                    col2.metric(label="Intraday SD", value=st.session_state.intrasd)
                    col3.metric(label="MAGE", value=st.session_state.mage)
                    col4.metric(label="J-Index", value=st.session_state.jindex)
                    col1.metric(label="LBGI", value=st.session_state.lgbi)
                    col2.metric(label="HBGI", value=st.session_state.hbgi)
                    col3.metric(label="CONGA24", value=st.session_state.conga)
                    col4.metric(label="GMI", value=st.session_state.gmi)
